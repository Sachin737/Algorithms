...
public:
	 ...
	 */
	Fibonacci_Heap *Fib_Heap_Insert(Fibonacci_Heap *H, int val)
	{
	...
	node *Fib_Heap_Find(Fibonacci_Heap *H, int x)
	{
		return fib_find(H->minNode, x);
	}
	 ...
	 */
	void Fib_Heap_Decrease_key(Fibonacci_Heap *H, int val, int k)
	{
		node *x = Fib_Heap_Find(H, val);
	 ...
	 * If operation is successful returns true otherwise false.
	 */
	void Fib_Heap_Delete_key(Fibonacci_Heap *H, int k)
	{
		Fib_Heap_Decrease_key(H, k, INT32_MIN);
		Extract_Min(H);
	}
